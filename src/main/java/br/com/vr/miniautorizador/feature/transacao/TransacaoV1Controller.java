...
public class TransacaoV1Controller implements TransacaoV1Doc {
    ...
    @PostMapping
    public ResponseEntity<TransacaoStatusEnum> createdTransacao(@Valid @RequestBody TransacaoDTO dto) {
        TransacaoStatusEnum transacaoStatus = transacaoService.validateTransacao(dto);

        return transacaoStatus != null
                ? ResponseEntity.status(422).body(transacaoStatus)
                : ResponseEntity.status(201).body(transacaoService.updateSaldoCartao(dto));
    }
}
